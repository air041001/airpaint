...
**不要只"代码改成功"就视为完成。** 阶段完成 = 验证通过 + 受影响的现状文档同步（如有）+ push；不要求每阶段新增 DEVLOG 或 ADR。
...
2. **验证标准**：按 §13 运行与改动匹配的检查，并核对受影响文档；纯文档改动检查内容、引用与 diff 即可。Prompt Engine 使用相关确定性回归，画质结论按人眼验收，不恢复无目标批量 baseline。
...
**Git 提交记录实现细节，DEVLOG 记录阶段结果，decisions 记录重要取舍。允许一次正常开发只提交代码，不新增日志或 ADR。** 不按对话、任务、commit 或测试轮次机械追加文档。

- **现状文档**：接口、架构、运维方式或使用方法变化时，只更新对应文档。BUILDHANDOFF 仅在项目级能力、验证边界或接手注意事项变化时更新；ROADMAP 仅在未来事项变化时更新。不将同一实现过程重复复制到各文档。
- **DEVLOG**：仅记录完整功能交付、用户可感知的重要修复、部署变化或值得长期保留的验证结论。同一目标下的调整和修复合并成阶段记录，通常 3～6 句话，写清变化、结果和未验证边界；测试只概述覆盖范围与结论，不罗列全部用例、函数和字段。普通参数调整、补测试、文件移动、注释和不改变行为的重构通常只进 Git。
- **decisions**：只有未来维护者需要理解“为什么采用此方案而放弃合理替代方案”时才新增。典型范围为产品边界、重要接口约定、数据真相源、关键依赖或推翻旧决定。普通 bug 修复、沿用既有设计的实现与小幅参数调整不新增 D 编号；不得为满足闭环而编造候选方案。
- **阅读**：新 Agent 默认读 AGENTS 与 BUILDHANDOFF，再按具体问题检索相关 ADR/日志；不要求通读全部历史。
- **历史整理**：优先压缩重复过程或合并同目标流水记录，保留原编号、日期、修订关系和关键证据入口。不得把“当时未验收”改写成“已完成”，也不得把后来推翻的结论伪装成一直正确。需要移动归档时保留原入口到新位置的映射；归档不按每次任务创建新文件。

本规约的文字维护不自动触发新日志或 ADR；提交记录足以说明普通文档调整。
...
| `docs/decisions.md` | 值得长期保留的重要设计取舍，新增门槛见 §15 |
| `docs/DEVLOG.md` | 阶段结果与重要验证结论，不作为逐次工作流水账 |
| `ROADMAP.md` | 未来准备做什么。只保留当前仍有效的路线 |
| `docs/workflow-anatomy.md` | ComfyUI / workflow 底层技术知识与踩坑记录 |
| `README.md` | 给开发者/使用者看的项目入口 |

### Decisions 格式建议（按需填写，不要求八段模板）

```text
Dxx. 标题

决定与原因：解决什么问题，为什么选择此方案。
代价与边界：放弃了什么，哪些结论尚未验证。
修订关系与证据：仅列相关旧 D 编号、必要验证或稳定入口。
```
