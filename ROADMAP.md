...
### 网站 MVP 细节优化（第一批已完成 2026-08-23）
- [x] 画布优先的一屏桌面布局：压缩输入区与历史带，AI 理解栏收窄，移动端保持纵向可用
- [x] 尺寸从平铺三选一改为点击展开、分组选择、选中即收起；删除 `1216x832`
- [x] 增加四个标准档和两个高分辨率实验档，高分辨率显式提示更慢/显存压力
- [x] RTX 4060 Laptop 8GB 实测 `1024x1536` 成功；超时按像素面积伸缩，前端改为 2～5 分钟预期
- [ ] 后续按真实使用继续处理 LoRA 中文摘要、空/错状态、暗房与历史作品细节，不启动整页换皮
