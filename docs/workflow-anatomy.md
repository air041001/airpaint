...
| 32 | PrimitiveInt | ImpactSwitch 42 的 select (值 2) |
| 42 | ImpactSwitch | latent 路由: input1=56(EmptyLatent/txt2img), input2=33(VAEEncode/img2img) |

> build_prompt: 传了 `image_filename` -> `set_input("switch_node","select",2)` 走 img2img; 否则默认 txt2img。
...
| img2img | 0->31->33->42 input2 | ✅ 活跃 (build_prompt 切 select=2) |
...
## API 耦合 (我们只动这些)
- **注入**: `54`(正向 text) / `6`(seed, img2img denoise) / `56`(width,height) / `5`(loras) / `0`(image_filename) / `42`(switch select=2 img2img)
