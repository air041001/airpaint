...
同时将 Composer 校验失败、上游服务失败、异常响应与浏览器传输失败/取消分开呈现，保留 HTTP 状态且不自动重试。mock 验收执行真实 CLIENT 响应解析及参考图→Composer→预览/最终化链：合法多格一次通过，单格和无模板各两次拒绝，模板正向与最终负面保留；P2B 19 项用法、26 项运行时/API、58 项 Prompt unit 和前端检查通过。未改生产 Registry/数据库，未调用真实模型/GPU；画面质量仍待用户实测。
