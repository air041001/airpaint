...
> 只记录阶段结果和重要验证结论，新增门槛见 `AGENTS.md` §15。同一目标的调整合并记录，普通工作细节留在 Git。
> 以下为按时间保留的历史，不代表全部结论仍适用；当前状态见 `BUILDHANDOFF.md`，无需接手时通读全文。
...
## 第 58 条 2026-08-28 - 图生图尺寸同步与风格预览第二版
...
## 第 67 条 2026-08-31 - 参考契约、Img2Img 与源图分支迭代（验收中）

按用户批准的 D59 计划对齐新模块结构：Vision 改为三档范围的结构化观察，统一交 Composer 合并用户/LoRA；上传区分 768 分析图与 1536 生成图；图生图增加重绘预设、保留/裁切和比例提示，工作流 seed 在入队前保存。节点 31 按本机 KJNodes INPUT_TYPES 与执行源码核对后只改适配参数，保留 width/height/image 上游连接。

暗房保存每轮完整最小快照，用 `source_job_id/parent_job_id` 从任一历史图分支；换一版无改动不调用模型，微调读取所选图并继承尺寸/seed。文字修改输出 `CHANGE_FIELDS` 更新既有 IR，不再拼接历史 raw。LoRA 链内固定，换 LoRA 回工坊新建。旧 image/vibe 留一个兼容周期，不引入数据库、区域提示词或局部重绘。

真实 Vision 首次省略了被排除的空字段，原严格十二字段检查误拒绝；现规范缺省空字段，仍拒绝未知字段或没有范围内有效观察。确定性测试覆盖范围、Vision→Composer、增量漂移拒绝、分支来源、seed/尺寸继承、图像适配连接、旧 API 与配额最后一张仍可查询。首次构图＋氛围图保留短发黄裙覆盖，但布局观察带入向日葵，已记录语义隔离并不完美，不当作全通过。

已删除 `.tools/eval_set/visual_director/` 32 个未跟踪一次性文件，不迁移，不能从 Git 恢复；结论留在 D55/D56/正式回归。`server/lora_registry.yaml` 不检查同步、不修改、不暂存、不推送。最多五张验收的逐图记录与用户结论见 BUILDHANDOFF；未获确认前不声明阶段完成。

本轮最终完成五张：构图参考的短发黄裙覆盖、完整参考淡蓝裙、0.35原图改傍晚、复用Prompt换seed、从旧图0.55改红裙。前两张各有局部参考偏差；第3张保形明显但改光很弱；第4张无LLM重编译且Prompt/IR字节相同，新seed获得明显傍晚；第5张CHANGE_FIELDS仅clothing、其余11字段不变，父任务/尺寸/seed都正确，但裙子仍蓝且风格漂移。真实Comfy历史确认red dress正向编码与0.55实际采样，无漏传。结论是工程链路可用，不是Img2Img编辑质量通过；不追加图片，不提交/推送为完成阶段，等待用户判断。

最终确定性检查：58项原Prompt回归、19项新增参考/迭代回归、6项LoRA Composition、Python compileall/pyflakes、前端2段脚本解析通过。浏览器完成上传、翻译、两种生成、换版、旧图分支、crop位置与比例推荐点击验证；节点31连接和1344×768源图分辨率由实际Comfy历史确认。验收页与图片保留在gitignored的server/images目录，不重建旧实验目录。
...
## 2026-09-09 · 最后一次前端收尾：沉浸展台（D61）

- 从用户选定的独立预览迁移到原生产请求逻辑：竖图完整适应高度、默认收起的 520px 桌面编辑器、三页签与独立手机布局。
- 全局历史与当前 session 的迭代分支分开呈现；查看旧图保留草稿，图片详情与新请求设置分开，后台任务不再抢占当前画面的源图 ID。
- 保留编译/手工 Prompt/LoRA/Profile/强度、参考/整图重绘、恢复核对、配额和登录。字体使用本机字体；未新增前端框架或改变后端协议。
- 最低验证包含前端内联脚本、D59/可靠性契约、草稿与源图状态回归和 Python 编译；浏览器独立验收范围见 `docs/demo.md`。不把夹具生成状态当成真实模型/GPU 或画质验证。
- `v1.0.0`/`9af5604` 保持封版基线；本次仅完成用户随后授权的前端收尾。Registry 与原有安全副本/stash 不动。

## 第 69 条 2026-09-09 - 人工删除输出后不再展示作品空壳
...
## 第 75 条 2026-09-13 - 补齐特殊 LoRA 模板执行

FComic 实测任务证明 Registry 引用和空警告只能说明资料可读，不能保证作者模板进入最终文本；旧规则还把“一次输出”误判为“禁止画布内多格”。现将用法模板独立于物理 Asset/Profile：用户显式选择后，Composer 负责补人物与场景，后端确定性写入模板正向骨架、负面追加和可选主体计数；明确单格只覆盖多格布局，人工 final 与用户负面仍最高优先。前端默认不套模板，展示实际正负骨架、作者姿势候选与仅供参考的尺寸；当前 CLIP 节点不展开 `{a|b}`，因此姿势候选不以花括号原样注入。预览、入队和任务快照均按当前不可变记录重验模板 ID。

本机 FComic 新增漫画基础、单男场景、群体场景三个模板并切换 Registry 引用，旧记录保留；写入前备份已校验，Registry 16 个 Asset 通过。确定性回归覆盖错误 IR 修正、明确单格、继承模板、普通 LoRA、列表→预览→任务→工作流字面一致；未调用真实模型/GPU，画面效果仍待用户人眼复测。

## 第 76 条 2026-09-14 - 修复多格模板被单镜头构图校验拒绝

特殊模板执行后，Composer 的通用可画性、多人构图和最终 tag 护栏仍把整张漫画的不同分镜当成同一镜头，导致合法的全身/可见脚部与上半身/近景组合修复两次后返回 502。现只从服务端已校验的所选模板派生可信布局模式，并贯穿 system/repair、解析和三处构图检查；可信多格跳过跨分镜无效的单镜头互斥与连续构图限制，但正文自称 multi-panel、无模板和明确单格不会绕过，人数、身份、资源及协议校验保持不变。

同时将 Composer 校验失败、上游服务失败、异常响应与浏览器传输失败/取消分开呈现，保留 HTTP 状态且不自动重试。mock 验收执行真实 CLIENT 响应解析及参考图→Composer→预览/最终化链：合法多格一次通过，单格和无模板各两次拒绝，模板正向与最终负面保留；P2B 19 项用法、26 项运行时/API、58 项 Prompt unit 和前端检查通过。未改生产 Registry/数据库，未调用真实模型/GPU。

用户随后有限实测：任务 `af813d0a79` 使用 `one_boy` 模板以 832×1216 成功完成，模板全部正向、多格布局与作者负面逐项存在于最终执行文本，用户认为图片达到可接受下限，据此结束模板执行回归验收。同期另有一次浏览器 `Failed to fetch`；访问日志尾部只见两次 translate 200 及后续任务/图片成功请求，缺少时间戳与请求关联 ID，不能严格配对或断言具体网络根因，作为独立传输稳定性问题保留。
