...
6. **确定性 Compiler / 可画性护栏**：代码补主体计数、清理精确角色的裸名变体、折叠完整逗号序列的机械复读；用户明确写 `2girls/2boys` 或双人/两人时，多主体 count 会覆盖冲突的单主体 count，并从最终 Prompt 与公开 IR 移除 `solo/solo focus`。性别仍来自用户 exact count 或 Composer 自身语义，不从 LoRA Profile 名猜测。用户明确写全身/完整可见时移除互斥近景。模型补全若同时发明多个手部/服装操作，或用 `upper body/close-up` 承诺裙摆、髋部、大腿等画外交互，会退回 Composer 改为牛仔镜头/四分之三身或删减动作。新文本路径仍不继承旧 `_prepare_painter_tags()` 的自动裸体、固定景别或画风删除。未知角色继续从 `IR.subject` 经 Danbooru exact 验证后写 auto cache。
