...
界面由登录、沉浸展台和同一视觉布局的迭代模式组成。日间使用白色/冷灰与蓝色操作色，夜间使用蓝灰底色。主题偏好保留原有存储键，旧 paper/graphite 值继续兼容；字体改用本机中文系统字体、Georgia 字标和 Consolas 参数字体。
结果图片占用视口剩余高度，横图与竖图均以 `object-fit: contain` 完整显示，可点击或键盘打开大图。桌面编辑器默认收起，展开为 520px 面板，分「画面构思 / Prompt / 成像设置」三个互斥页签；编辑内容内部滚动，操作按钮留在面板底部。手机使用独立宽度和高度约束，不沿用桌面窄栏。
「所有作品」是独立的全局历史对话框；普通画布不显示迭代缩略条。「继续创作」恢复或创建真实 session，只有此时显示「本次迭代」，父子关系按 `parent_job_id` 展示。旧 app/darkroom 请求状态机仍分开，不把历史列表伪装成某张图的演化线。
