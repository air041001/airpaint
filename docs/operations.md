...
| `done` | 图片已保存且可读取 | 无 |
| `deleted` | 本地输出文件已确认删除；仅作内部关系/配额墓碑，不在作品历史展示 | 无 |
...
人工删除 `server/images/<job>.png` 后，下一次服务启动、历史刷新或该任务查询会由后端确认文件缺失，并从“所有作品”和对应会话中隐藏该图及参数。数据库保留不可见墓碑以维持父子关系和用量审计，原次数不返还。不要根据浏览器破图图标判断删除：网络异常会保留记录，登录失效返回 401，只有服务端确认本地文件不存在才执行隐藏。输出图响应不再允许一小时浏览器缓存。
