...
`v1.0.0`（`9af5604`）保持为历史封版基线，旧 tag 不变。2026-09-09 按用户授权完成沉浸展台前端收尾；此后用户解除封版并授权有限实验线：最终正负文本控制、LoRA 用法资料入库、中文编译消费用法资料、用法资料接入 onboarding。FComic 真实试用暴露模板未执行后，又完成了独立模板选择与确定性正负骨架修复；真实模型/GPU 与图像质量仍待用户复测。新模型、RAG、微调、多 Agent、自动改图、新 workflow、微服务和前端框架迁移仍不在范围内。
